...
const App: React.FC = () => {
  ...
  // Logika pro zobrazení a skrytí "Success Overlay" po 5 sekundách
  useEffect(() => {
    if (isLevel) {
      setShowSuccessOverlay(true);
      const timer = setTimeout(() => {
        setShowSuccessOverlay(false);
      }, 5000); // Skrýt po 5 sekundách
  ...
  if (isIOS && permissionGranted === false) {
      return (
          <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-vw-dark text-center">
              ...
              <button onClick={() => window.location.reload()} className="mt-8 bg-gray-700 px-6 py-3 rounded-xl">Zkusit znovu</button>
          </div>
      )
  }
  ...
  return (
    <div className="min-h-screen bg-vw-dark flex flex-col items-center py-6 px-4 relative overflow-hidden selection:bg-vw-accent selection:text-white">
      ...
      {feedback && (
        <div className={`fixed top-20 left-1/2 z-50 flex items-center gap-3 px-6 py-3 rounded-full shadow-2xl backdrop-blur-md border animate-fade-in-up whitespace-nowrap ${
            feedback.type === 'success' 
              ? 'bg-vw-success/20 border-vw-success text-vw-success' 
      ...
      {isIOS && permissionGranted === null && (
        <PermissionModal onGrant={requestPermission} />
      )}

      {/* Header */}
      <header className="mb-6 text-center z-10 shrink-0">
        <h1 className="text-3xl font-black tracking-tight text-white mb-1">
          T6 <span className="text-vw-accent">LEVELER</span>
        </h1>
        <p className="text-gray-400 text-sm">Volkswagen T6 / T6.1 California</p>
      </header>
      ...
      <main className="relative w-full max-w-[360px] aspect-[4/5] bg-gray-800/30 rounded-3xl border border-gray-700 shadow-2xl flex items-center justify-center mb-6 shrink-0 overflow-hidden">
        ...
        {/* Success Overlay - Zobrazí se, když je vše 0, zmizí po 5s */}
        {showSuccessOverlay && (
